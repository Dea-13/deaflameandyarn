...
@Component({
  selector: 'app-used-status',
  templateUrl: './used-status.component.html',
  styleUrls: ['./used-status.component.scss']
})
export class UsedStatusComponent implements OnInit {
  // Public
  public rows = [];
  ...
  makeTable(){
    this.columns = [
      ...
      { name: this.translateSnackBar.names, prop: "name" },
      { name: this.translateSnackBar.press, prop: "name" },
  ...
  getRequest(count, searchValue) {
    this.limit = count;
    // this.matrixService
      // .getUsedByStatus(this.offset, this.limit, searchValue)
      // .subscribe((data) => {
      //   this.rows = data.list;
