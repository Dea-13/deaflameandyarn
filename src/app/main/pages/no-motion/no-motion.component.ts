...
@Component({
  selector: 'app-no-motion',
  templateUrl: './no-motion.component.html',
  styleUrls: ['./no-motion.component.scss'],
})
export class NoMotionComponent implements OnInit {
  // Public
  public rows = [];
  ...
  makeTable() {
    this.columns = [
      ...
      { name: this.translateSnackBar.lastTransaction, prop: 'name' },
      ...
      { name: this.translateSnackBar.productionKg, prop: 'name' },
      { name: this.translateSnackBar.renewal, prop: 'name' },
    ];
  ...
  getRequest(count, searchValue) {
    this.limit = count;
    // this.matrixService
    // .getNoMotionMatrix(this.offset, this.limit, searchValue)
    // .subscribe((data) => {
    //   this.rows = data.list;
