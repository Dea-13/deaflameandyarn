...
export class StatedComponent implements OnInit {
  ...
  constructor(
    ...
    } else if(this.router.url == '/api/blocked-die') {
      this.statusId = 32;
      this.displayedColumns = [
        'dieId', 'profileId', 'primaryResourceName', 'producerName', 'diameter',
        'thickness', 'clientName','countInUse', 'KgNet', 'totalWeight', 'channels', 'placeSklad', 'lastTransaction',
  ...
  getRequest() {
    ...
    switch (this.router.url) {
      ...
      case '/api/blocked-die': { url = this.matrixService.getBlockedDie(this.offset, this.limit, this.statusId, this.arrFilters[0].model, this.arrFilters[1].model, this.arrFilters[2].model, this.arrFilters[3].model, this.arrFilters[4].model, this.arrFilters[5].model, this.arrFilters[6].model, this.arrFilters[7].model, this.formatDate(this.selDateOrderForm.controls.selDateOrder.value), this.formatDate(this.selDateConfirmForm.controls.selDateConfirm.value), this.formatDate(this.selDateExpedForm.controls.selDateExped.value), this.formatDate(this.selDateScrappedForm.controls.selDateScrapped.value), this.grM, this.lastModified, this.bmwinventorynumber, this.dieLiveQty, this.orderType, this.orderBy); } break;
    }
    url.subscribe((data) => {
      this.rows = data.list;
      this.countTable++;
  ...
  getFilters(ind, action) {
    ...
    switch (this.router.url) {
      ...
      case '/api/blocked-die': { url = this.matrixService.getFiltersBlocked(this.urls[i].name, this.keyword, this.statusId, this.arrFilters[0].model, this.arrFilters[1].model, this.arrFilters[2].model, this.arrFilters[3].model, this.arrFilters[4].model, this.arrFilters[5].model, this.arrFilters[6].model, this.arrFilters[7].model); } break;
    }
